...
def load_mvnn():
    print(f"[{time.strftime('%X')}] Loading MVNN Checkpoint...", flush=True)
...
def evaluate_mvnn(params, fcn_f, x_w, v_w, x_m, v_m, x_c, v_c, name=""):
    print(f"[{time.strftime('%X')}] Starting JAX Evaluation for {name} in chunks...", flush=True)
    t0 = time.time()
    ...
    t1 = time.time()
    print(f"[{time.strftime('%X')}] Finished JAX Evaluation for {name} in {t1-t0:.2f} seconds.", flush=True)
    return float(error_total)

def get_predicted_masks(seed, clf, scaler, multipliers):
    print(f"[{time.strftime('%X')}] Loading RF Model and Predicting Labels...", flush=True)
    ...
    m_mask = (preds == 1)
    c_mask = (preds == 2)
    print(f"[{time.strftime('%X')}] Predicted: {w_mask.sum()} Workers, {m_mask.sum()} Managers, {c_mask.sum()} CEOs.", flush=True)
    return w_mask, m_mask, c_mask

def main():
    print(f"[{time.strftime('%X')}] Process Started.", flush=True)
    fcn_f, mvnn_params = load_mvnn()
    
    print(f"[{time.strftime('%X')}] Loading Hybrid RF Model Artifacts...", flush=True)
    ...
    seed = 70
    print(f"\n[{time.strftime('%X')}] Testing SINGLE SEED: {seed}", flush=True)
    ...
    err_gt = evaluate_mvnn(mvnn_params, fcn_f, x_w_gt, v_w_gt, x_m_gt, v_m_gt, x_c_gt, v_c_gt, name="Ground Truth")
    
    # 2. Predicted
    w_mask, m_mask, c_mask = get_predicted_masks(seed, clf, scaler, multipliers)
    ...
    err_pred = evaluate_mvnn(mvnn_params, fcn_f, x_w_pred, v_w_pred, x_m_pred, v_m_pred, x_c_pred, v_c_pred, name="Predicted Labels")
    
    print(f"\n[{time.strftime('%X')}] TEST FINISHED! GT Error: {err_gt:.5f}, Pred Error: {err_pred:.5f}", flush=True)
